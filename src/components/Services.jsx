...
const Services = () => {
  ...
  return (
    <Box id="services" sx={{ py: 12, bgcolor: 'white' }} className="reveal-section max-sm:py-6">
        ...
        <Grid container spacing={4} sx={{ mb: 8 }} className="max-sm:gap-4 max-sm:mb-5">
          {services.map((service, index) => (
            <Grid item xs={12} md={4} key={index} className="max-sm:w-full mobile-animate-in">
              <Card className="reveal-card mobile-services-card h-full text-center p-8 border-2 border-[#e2e8f0] hover:border-[#3b82f6] max-sm:p-5 max-sm:w-full">
        ...
        <Grid container spacing={3} className="max-sm:gap-4">
          {benefits.map((benefit, index) => (
            <Grid item xs={12} md={4} key={index} className="max-sm:w-full">
              <Card className="reveal-card text-center py-6 bg-[#0f172a] border-none hover:scale-105 hover:bg-[#1e293b] [&_*]:!text-white max-sm:py-4">
                <CardContent>
